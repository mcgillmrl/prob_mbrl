...
class LunarLander(gym.Env, EzPickle):
    ...
    def step(self, action):
        ...
        if (self.continuous and action[0] > 0.0) or (not self.continuous
            ...
            if self.continuous:
                m_power = (
                    np.clip(action[0], 0.0, 1.0) + 1.0) * 0.5  # 0.5..1.0
            ...
            oy = -tip[1] * (
                4 / SCALE + 2 * dispersion[0]) - side[1] * dispersion[1]
            ...
            p.ApplyLinearImpulse((ox * self.MAIN_ENGINE_POWER * m_power,
                                  oy * self.MAIN_ENGINE_POWER * m_power),
                                 impulse_pos, True)
            self.lander.ApplyLinearImpulse(
                (-ox * self.MAIN_ENGINE_POWER * m_power,
                 -oy * self.MAIN_ENGINE_POWER * m_power), impulse_pos, True)
        ...
        if (self.continuous
            ...
            p = self._create_particle(0.7, impulse_pos[0], impulse_pos[1],
                                      s_power)
            p.ApplyLinearImpulse((ox * self.SIDE_ENGINE_POWER * s_power,
                                  oy * self.SIDE_ENGINE_POWER * s_power),
                                 impulse_pos, True)
            self.lander.ApplyLinearImpulse(
                (-ox * self.SIDE_ENGINE_POWER * s_power,
                 -oy * self.SIDE_ENGINE_POWER * s_power), impulse_pos, True)
...
def heuristic(env, s):
    ...
    hover_targ = 0.55 * np.abs(
        s[0])  # target y should be proporional to horizontal offset
